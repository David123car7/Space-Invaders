...
void GameController::Update() {
  ...
  invadersController.MoveInvaders(Constants::INVADERS_SPEED,
                                  Constants::BORDERS_GAP,
                                  Constants::INVADER_SIZE);
  ufo.MoveUfo(Constants::UFO_SPEED);

  invadersController.HandleAnimations(texturesController.GetInvaderTextureA0(),
...
void GameController::Render() {
  ...
  uiController.DisplayUI(Constants::UI_TEXT_SIZE);
}
...
void GameController::HandleInput() {
  ...
  if (IsKeyDown(KEY_RIGHT) &&
      playerPosX < Constants::WINDOW_WIDTH -
                       texturesController.GetPlayerWidth() -
                       Constants::BORDERS_GAP)
    player.MoveRight();
  if (IsKeyDown(KEY_LEFT) && playerPosX > 0 + Constants::BORDERS_GAP)
    player.MoveLeft();
...
void GameController::RefreshUI() {
  uiController.SetScoreText(Constants::UI_SCORE_TEXT + std::to_string(score));
  uiController.SetLivesText(Constants::UI_LIVES_TEXT +
                            std::to_string(player.GetLives()));
}

void GameController::RestartGame() {
  invadersController.ResetInvaders();
  player.ResetPlayer(Constants::PLAYER_LIVES);
  score = 0;
  Start();
}
  ...
  for (int i = 0; i < bullets.size(); i++) {
    for (int j = 0; j < invaders.size(); j++) {
      ...
      if (CheckCollisionCircles(
              bullets[i].GetPosition(), (float)Constants::BULLET_WIDTH / 2,
              invaders[j].GetPosition(), (float)Constants::BULLET_HEIGHT / 2)) {
        bulletsPlayerController.RemoveBullet(i);
        invadersController.KillInvader(
            j, texturesController.GetInvaderExplosionTexture());
...
void GameController::CheckCollisionsPlayerAndInvadersBullets() {
  ...
  for (int i = 0; i < bullets.size(); i++) {
    if (CheckCollisionCircles(bullets[i].GetPosition(),
                              (float)Constants::BULLET_WIDTH / 2,
                              player.GetPosition(),
...
void GameController::CheckCollisionsPlayerBulletsAndUfo() {
  ...
  for (int i = 0; i < bullets.size(); i++) {
    if (CheckCollisionCircles(
            bullets[i].GetPosition(), (float)Constants::BULLET_WIDTH / 2,
            ufo.GetPosition(), (float)texturesController.GetUfoWidth() / 2)) {
...
void GameController::CheckCollisionsPlayerAndInvaders() {
  ...
  for (int i = 0; i < invaders.size(); i++) {
    Vector2 invaderPos{
        invaders[i].GetPositionX() + (float)Constants::INVADER_SIZE / 2,
        invaders[i].GetPositionY() + (float)Constants::INVADER_SIZE / 2};
    if (CheckCollisionCircles(playerPos,
                              (float)texturesController.GetPlayerWidth() / 2,
                              invaderPos, Constants::INVADER_SIZE)) {
...
void GameController::SpawnPlayer() {
  Vector2 playerStartPos{(float)Constants::WINDOW_WIDTH / 2 -
                             (float)texturesController.GetPlayerWidth() / 2,
                         (float)Constants::WINDOW_HEIGHT - 150 -
                             (float)texturesController.GetPlayerWidth() / 2};
  player.SetPosition(playerStartPos);
}

void GameController::SpawnInvaders() {
  Vector2 invaderStartPos{(float)Constants::WINDOW_WIDTH / 2 -
                              Constants::INVADER_SIZE *
                                  Constants::INVADERS_X_SIZE,
                          (float)Constants::WINDOW_HEIGHT / 2 - 80};
  invadersController.SpawnInvaders(
      invaderStartPos, texturesController.GetInvaderTextureA0(),
      texturesController.GetInvaderTextureB0(),
      texturesController.GetInvaderTextureC0(), 1, Constants::INVADERS_COLOR,
      Constants::DEATH_ANIMATION_TIME);
}

void GameController::SpawnUfo() {
  Vector2 startPos = {-300, 150};
  ufo = {startPos,
         texturesController.GetUfoTexture(),
         Constants::UFO_COLOR,
         Constants::UFO_STOP_TIMER,
         true,
         false};
}

void GameController::PlayerShootInput() {
  float posX = player.GetPositionX() + player.GetWidth() / 2 -
               (float)Constants::BULLET_WIDTH / 2;
  float posY = player.GetPositionY() - texturesController.GetBulletHeight();
  Vector2 bulletStartPos{posX, posY};
  bulletsPlayerController.SpawnBullet(bulletStartPos, 1000.f,
                                      texturesController.GetBulletTexture(),
...
void GameController::InvadersShoot() {
  if (!bulletsInvaderController.GetCanShoot())
    return;
  Vector2 invBulletPos = invadersController.GetRandomInvaderBulletVector(
      texturesController.GetBulletHeight());
  bulletsInvaderController.SpawnBullet(invBulletPos, 1000.f,
                                       texturesController.GetBulletTexture(),
                                       Constants::BULLET_COLOR);
}

void GameController::UfoShoot() {
  if (!bulletsUfoController.GetCanShoot())
    return;
  Vector2 ufoBulletPos =
      ufo.GetBulletPosition(texturesController.GetBulletHeight());
  bulletsInvaderController.SpawnBullet(ufoBulletPos, 1000.f,
                                       texturesController.GetBulletTexture(),
                                       Constants::BULLET_COLOR);
}
